...
template <bit_count_t NumInputs> constexpr bit_count_t num_possible_outputs() {
  static_assert(NumInputs < (sizeof(bit_count_t) * 8u),
...
constexpr auto invoke_function(logic_function<OutputN, InputNs...> function,
                               const bits<detail::sum<InputNs...>()> input_bits,
                               xtl::index_sequence<Indexes...>) {
  constexpr auto bit_start_offsets = detail::compute_bit_offsets<InputNs...>();

  static_assert(
      bit_start_offsets.size() == sizeof...(InputNs),
      "The number of bit offsets should be the same as the number of inputs.");

  constexpr xtl::array<bit_count_t, bit_start_offsets.size()> bit_lengths{
      InputNs...};

  return function(
      input_bits.template span<xtl::get<Indexes>(bit_start_offsets),
                               xtl::get<Indexes>(bit_start_offsets) +
                                   xtl::get<Indexes>(bit_lengths)>()...);
}
...
constexpr auto compile(logic_function<OutputN, InputNs...> function) {
  ...
  for (auto i = 0u; i < NumPossibleOutputs; ++i) {
    output_bits[i] = detail::invoke_function(
        function, i, xtl::make_index_sequence<sizeof...(InputNs)>());
  }
