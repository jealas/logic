...
public:
  ...
  constexpr BitStorage &operator[](const size_t i) noexcept {
    return storage_[i];
  }
  constexpr const BitStorage &operator[](const size_t i) const noexcept {
    return storage_[i];
  }

  constexpr BitStorage const *begin() const noexcept { return &storage_; }
  constexpr BitStorage const *end() const noexcept {
    return &storage_[NumOutputs];
  }
